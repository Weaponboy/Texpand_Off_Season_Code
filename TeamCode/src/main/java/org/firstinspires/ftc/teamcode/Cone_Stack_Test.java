...
@TeleOp
public class Cone_Stack_Test extends OpMode {
    ...
    public void loop() {
        ...
        if(gamepad1.a && Base_Pivot.getPosition() == 0){
            Base_Pivot.setPosition(0.85); //lift up base griper if it is down
        ...
        if (gamepad1.x) {
            ...
            if(Toppos == 1){
                Base_Lift.setPosition(0);
            }else if(Toppos == 2){
                Base_Lift.setPosition(1);
            }
            if(Toppos > 2){
                Toppos = 0;
            }
    ...
    public void init() {
        ...
        Base_Lift = hardwareMap.get(Servo.class,"Base_Lift");
